...
public class NMSArmorStand extends EntityArmorStand implements TNLArmorStand, SlotWrapper {
    ...
    public void setHeadPose(@Nullable Pose pose) {
        if (pose != null) super.setHeadPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setBodyPose(@Nullable Pose pose) {
        if (pose != null) super.setBodyPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setLeftArmPose(@Nullable Pose pose) {
        if (pose != null) super.setLeftArmPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setRightArmPose(@Nullable Pose pose) {
        if (pose != null) super.setRightArmPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setLeftLegPose(@Nullable Pose pose) {
        if (pose != null) super.setLeftLegPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setRightLegPose(@Nullable Pose pose) {
        if (pose != null) super.setRightLegPose(new Vector3f(pose.getPitch(), pose.getYaw(), pose.getRoll()));
    }

    @Override
    public void setCustomName(@Nullable String customName) {
        super.setCustomName(customName != null ? CraftChatMessage.fromString(customName)[0] : null);
    }
