...
public class NMSFallingBlock extends EntityFallingBlock implements TNLFallingBlock {
    ...
    public NMSFallingBlock(@Nonnull Location location, @Nonnull Material type) {
        super(((CraftWorld) Objects.requireNonNull(location.getWorld())).getHandle(), location.getX(), location.getY(), location.getZ(), ((CraftBlockData) type.createBlockData()).getState());
        ticksLived = 1;
        persist = true;
        setNoGravity(true);
    }
